...
4. **최대 위반 우선 알림 (Max Excess Priority)**: 다중 급부 위반 시, 설계사에게 혼란을 주지 않기 위해 **초과 금액이 가장 큰 급부**를 우선적으로 가시화하고 조정 로그의 헤드라인으로 채택한다. (v1.5 Update)
...
def find_worst_violation(riders, benefits, caps, ignore_set=None):
    """
    ...
    for benefit in benefits:
        if ignore_set and benefit.id in ignore_set: continue

        # 해당 급부에 기여하는 금액 합산 + 기계약 금액(BaseAmount)
        total_contribution = benefit.base_amount
        ...
        # 급부 캡과 비교
        if total_contribution > benefit.cap:
            excess = total_contribution - benefit.cap
            if excess > max_excess:
                max_excess = excess
                worst_violation = (benefit.id, excess)

    return worst_violation  # 없으면 None
```
...
### 6.3 시나리오 4: 신암진단 복합 정책 테스트 (v1.5)
- **배경**: 암진단특약과 신암진단특약이 공통 급부에 속하면서도, 신암진단특약에만 별도의 '기계약 합산 5,000만원' 테두리가 있는 경우.
- **검증**: 신암진단특약 가입 시 기존 기계약 3,000만원과 합산하여 5,000만원을 넘는지 독립적으로 검증해야 함.
- **결과**: 유방전립선암 등 다른 암특계약이 신암진단 전용 한도에 침범하지 않도록 매핑 무결성 확인.
```
